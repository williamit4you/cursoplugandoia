...
> = {
  SCRAPE_MEDIA: {
    ...
    summary: "Busca a pagina da Shopee, extrai titulo, descricao, imagens, video original e prepara uma ideia de engajamento.",
...
export default function ShopeePipelinePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/engajamento-pipeline/items?${qs.toString()}`, { cache: "no-store" });
      const data = await res.json();
      setItems(Array.isArray(data) ? data : []);


      const cfgRes = await fetch("/api/engajamento-pipeline/config", { cache: "no-store" }).catch(() => null);
      const cfgData = cfgRes ? await cfgRes.json().catch(() => null) : null;
      setCronConfig(cfgData && typeof cfgData === "object" ? cfgData : null);

      const cronRes = await fetch("/api/engajamento-pipeline/internal-cron", { cache: "no-store" }).catch(() => null);
  ...
  const loadConfig = async () => {
    ...
    try {
      const res = await fetch("/api/engajamento-pipeline/config", { cache: "no-store" });
      const data = await res.json();
      setConfig(data);
      setConfigDraft({
  ...
  const saveConfig = async () => {
    ...
    setConfigLoading(true);
    try {
      const res = await fetch("/api/engajamento-pipeline/config", {
        method: "POST",
  ...
  const toggleCron = async (enabled: boolean) => {
    ...
    await fetch("/api/engajamento-pipeline/config", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  const openDetail = async (item: ColetaItem) => {
    ...
    try {
      const full = await fetch(`/api/engajamento-pipeline/items/${item.id}`, { cache: "no-store" })
        .then((r) => r.json())
        .catch(() => null);
      ...
      const res = await fetch(`/api/engajamento-pipeline/items/${item.id}/events?take=200`, { cache: "no-store" });
      const data = await res.json();
  ...
  const cronUrl = useMemo(() => {
    ...
    return "/api/engajamento-pipeline/cron";
  }, []);
  ...
  const focusStep = async (stepName: string) => {
    ...
    setEventsLoading(true);
    try {
      const res = await fetch(`/api/engajamento-pipeline/items/${selected.id}/events?take=200&stepName=${encodeURIComponent(stepName)}`, {
        cache: "no-store",
      });
      const data = await res.json();
      setEvents(Array.isArray(data) ? data : []);
  ...
  const patchItem = async (id: string, patch: any) => {
    const res = await fetch(`/api/engajamento-pipeline/items/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  const runManualOnce = async () => {
    ...
    try {
      const res = await fetch("/api/engajamento-pipeline/manual-run", { method: "POST", cache: "no-store" });
      ...
      if (itemId) {
        const latest = await fetch(`/api/engajamento-pipeline/items/${itemId}`, { cache: "no-store" })
          .then((r) => r.json())
          .catch(() => null);
  ...
  const runManualPublisher = async () => {
    ...
    try {
      const res = await fetch("/api/engajamento-pipeline/manual-publisher", { method: "POST", cache: "no-store" });
      const data = await res.json().catch(() => ({}));
      ...
      <Card sx={{ border: "1px solid #cbd5e1", bgcolor: "#ffffff", boxShadow: "0 1px 2px rgba(15,23,42,0.06)" }}>
        <CardContent>
          ...
          <Typography variant="caption" sx={{ mt: 1.5, display: "block", color: "#475569" }}>
            Endpoint de diagnostico: <code>/api/engajamento-pipeline/internal-cron</code> • endpoint manual: <code>/api/engajamento-pipeline/cron</code>
          </Typography>
        </CardContent>
      </Card>
            ...
            <Box className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <TextField
                  ...
                  helperText="Intervalo do cron /api/engajamento-pipeline/cron."
                  size="small"
                  sx={darkFieldSx}
                  slotProps={{ inputLabel: { shrink: true } }}
                />
