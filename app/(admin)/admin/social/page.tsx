...
export default function SocialPostsDashboard() {
  ...
  const requeueOldPosts = useCallback(async () => {
    if (!confirm("Reagendar todos os posts antigos que ainda nao foram publicados? Eles serao distribuidos no futuro em intervalos de 2 horas.")) return;
  ...
  const handleAction = async (id: string, action: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Falha na acao");
  ...
  const checkOrPublish = async (post: any) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Falha ao processar publicacao");
      
      if (data.stillProcessing) {
        toast.info("A Meta ainda esta processando o video no container assincrono. Aguarde um minuto.");
      } else {
        toast.success("Publicacao executada/atualizada com sucesso!");
      }
      fetchPosts(true);
    } catch (err: any) {
      toast.error(err.message);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title="Reagendar posts antigos nao publicados em intervalos de 2 horas"
          >
      ...
      <div className={`grid grid-cols-1 ${lockedPlatform ? "md:grid-cols-3" : "md:grid-cols-4"} gap-4 bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm`}>
        ...
        <div className="relative">
          ...
          >
            ...
            <option value="PROCESSING_MEDIA">Processando Midia</option>
          </select>
        </div>
                ...
                {expandedGroups.has(group.id) && (
                  <div className="bg-slate-50/50 border-t border-slate-100 p-5 space-y-3">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <ListIcon className="w-3.5 h-3.5 text-slate-400" /> Detalhes das Publicacoes
                    </h4>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                {p.scheduledTo ? `Agendado: ${new Date(p.scheduledTo).toLocaleString('pt-BR')}` : 'Nao agendado'}
                              </p>
                              {p.log && (
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-4 bg-white rounded-2xl border border-dashed border-slate-200">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-500 max-w-xs mx-auto">
                Tente ajustar os filtros ou aguarde as proximas automacoes gerarem postagens.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-between bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm">
        <div className="text-xs text-slate-500 font-bold">
          Pagina <span className="text-slate-800 font-black">{page}</span> de {Math.ceil(total / pageSize) || 1}
        </div>
