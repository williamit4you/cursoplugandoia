...
export default function EngajamentoShopeePage() {
  ...
  const loadColetas = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/engajamento-shopee");
      if (res.ok) {
  ...
  const handleScrape = async (id: string) => {
    try {
      ...
      const res = await fetch(`/api/engajamento-shopee/${id}/scrape`, { method: "POST" });
      if (!res.ok) {
        const data = await res.json();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/engajamento-shopee/${id}`, { method: "DELETE" });
      await loadColetas();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleSaveEdit = async () => {
    ...
    setSaveMsg(null);
    try {
      const res = await fetch(`/api/engajamento-shopee/${selectedColeta.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/engajamento-shopee/${videoColeta.id}/criar-video`, {
        method: "POST",
        body: form,
      });
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4">
            <TextField
              ...
              size="medium"
              variant="outlined"
              placeholder="Cole a URL do produto da Shopee para gerar uma ideia de engajamento"
              value={url}
            ...
            >
              ...
              >
                ...
                <TextField
                  ...
                  sx={fieldSx}
                  placeholder="Script de engajamento gerado pela IA"
                  helperText={editFields.aiPromptVendas ? "Roteiro salvo e pronto para ajuste manual." : "A IA ainda nao retornou a ideia de engajamento para este item."}
                />
                {saveMsg && (
