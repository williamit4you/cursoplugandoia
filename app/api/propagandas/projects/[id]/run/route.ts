...
export async function POST(req: NextRequest, ctx: { params: { id: string } }) {
  try {
    ...
    // 1. Verify project exists and is a product ad
    ...
    if (!genResult.ok) {
      return NextResponse.json({ error: genResult.error || "Failed to generate copy" }, { status: 500 });
    }
    ...
    console.error("[api/propagandas/projects/[id]/run POST]", error);
