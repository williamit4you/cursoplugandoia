...
export async function createFreeThumbnail(id: string, title: string, meta: LongFormMetadata) { const words = lines(title || "Marketing digital sem enrolacao"); const headline = words.map((line, i) => `<text x="100" y="${260 + i * 125}" fill="#fff" font-family="Arial Black,Arial" font-size="92" font-weight="900">${esc(line)}</text>`).join(""); const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720"><rect width="1280" height="720" fill="#080808"/><rect width="38" height="720" fill="#dc2626"/><circle cx="1110" cy="120" r="170" fill="#dc2626"/><circle cx="1180" cy="620" r="280" fill="#991b1b" opacity=".35"/><text x="100" y="112" fill="#fca5a5" font-family="Arial" font-size="30" font-weight="700" letter-spacing="4">MARKETING DIGITAL • ${meta.funnelStage || "TOPO"}</text>${headline}<rect x="100" y="615" width="315" height="62" rx="31" fill="#dc2626"/><text x="130" y="656" fill="#fff" font-family="Arial" font-size="29" font-weight="700">PLUGANDO IA</text></svg>`; return uploadBufferToMinio({ buffer: Buffer.from(svg), key: `long-form-marketing/thumbnails/${id}.svg`, contentType: "image/svg+xml" }); }
export async function findPexelsAssets(queries: string[], enabled: boolean) { if (!enabled) return []; const results = await Promise.all(queries.slice(0, 6).map(async (query) => ({ query, assets: await searchPexelsMedia(query, 1, "landscape") }))); return results.flatMap(({ query, assets }) => assets.map((asset) => ({ url: asset.url, source: "Pexels", query }))); }
