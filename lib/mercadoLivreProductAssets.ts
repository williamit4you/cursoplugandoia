...
function looksLikeImageUrl(value: string | null | undefined) {
  ...
  return /mlstatic\.com|mercadolivre\.com|mercadolibre\.com|frontend-minio|postgresqlpdf-minio/i.test(url);
}
...
export async function prepareMercadoLivreProductAssets(
  product: Pick<MercadoLivreProduct, "id" | "permalink" | "thumbnailUrl">,
  limit = 4
): Promise<MercadoLivrePreparedAsset[]> {
  ...
  const assets: MercadoLivrePreparedAsset[] = [];
  let index = 0;
  for (const imageUrl of candidates) {
    ...
    try {
      ...
      const key = `mercado-livre-products/${safeKeyPart(product.id)}-${index}.${ext}`;
