...
import { calculateTextSimilarity, validateSeoAngleDistinctness, validateSeoRelease } from "@/lib/seoGovernance";
...
export async function runSeoAgentPipeline(briefId: string) {
  const brief = await prisma.seoBrief.findUnique({ where: { id: briefId }, include: { product: { include: { briefs: true } }, opportunity: true } });
  ...
  const duplicate = existingPosts.map((post) => ({ ...post, score: calculateTextSimilarity(brief.title, post.title) })).sort((a, b) => b.score - a.score)[0] || null;
  const siblingAngles = validateSeoAngleDistinctness(
    brief.product.briefs.map((item) => ({
      angle: item.angle,
      title: item.id === brief.id ? brief.title : item.title,
      keyword: item.id === brief.id ? brief.primaryKeyword : item.primaryKeyword,
    })),
  );
  ...
  const context: any = { product: brief.product, brief, opportunity: brief.opportunity, releaseGate: gate, deterministicReview: { duplicate, duplicateBlocked: Number(duplicate?.score || 0) >= 0.75, sensitiveClaim, siblingAngles } };
  for (const role of Object.keys(ROLES) as (keyof typeof ROLES)[]) context[role] = await runAgent(role, context);
  const sensitive = Boolean(context.review?.requiresHumanReview || context.review?.sensitiveClaims?.length || context.deterministicReview.duplicateBlocked || context.deterministicReview.sensitiveClaim || !context.deterministicReview.siblingAngles?.ok);
