...
export async function startOperationRun(operationKey: OperationKey, metadata?: unknown) {
  try {
    ...
    const run = await prisma.operationRun.create({ data: { operationKey, metadataJson: safeJson(metadata) } });
    return run;
  } catch (error) {
...
export async function heartbeatOperationRun(runId: string | null | undefined, values?: { itemsFound?: number; itemsProcessed?: number; itemsSucceeded?: number; itemsFailed?: number; nextRunAt?: Date | null }) {
  if (!runId) return;
  try {
    await prisma.operationRun.update({ where: { runId }, data: { heartbeatAt: new Date(), ...values } });
  } catch (error) {
    console.warn("[observability] falha no heartbeat", error instanceof Error ? error.message : error);
  }
}
  ...
  try {
    await prisma.operationRun.update({
      ...
      data: {
        ...
        metadataJson: safeJson(result.metadata),
      },
    });
  } catch (error) {
    console.warn("[observability] falha ao finalizar OperationRun", error instanceof Error ? error.message : error);
  }
}
